...
def load_hdf5(filename):
    def create_npy_dataset(f, name, item):
...
def load_dump(filename):
    ...
    if not os.path.isfile(filename):
        raise ValueError("Input file '{}' doesn't exist.".format(filename))
    ...
    creation_date = data['__header__'].decode('utf8').split('Created on: ')[1]
...
def load_from_archiver(t_start, t_end):
    a = Archiver()
